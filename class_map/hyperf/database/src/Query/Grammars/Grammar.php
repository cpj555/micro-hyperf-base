...
class Grammar extends BaseGrammar
{
     ...
     * @param array $values
     */
    public function compileInsertOrIgnore(Builder $query, array $values): void
    {
    ...
    public function compileInsertOrUpdate(Builder $query, array $insertValues, array $updateValue)
    {
        $table = $this->wrapTable($query->from);
        ...
        $columns = $this->columnize(array_keys(reset($insertValues)));

        $insertParameters = collect($insertValues)->map(fn ($record) => '(' . $this->parameterize($record) . ')')->implode(', ');

        $updateParamters = collect($updateValue)->map(fn ($value, $key) => $this->wrap($key) . ' = ' . $this->parameter($value))->implode(', ');

        return "insert into {$table} ({$columns}) values {$insertParameters} on duplicate key update {$updateParamters} ";
    }
    ...
    public function compileUpdateBatch(Builder $query, array $fields, array $arguments, string $keyName)
    {
        ...
        foreach ($fields as $field) {
            $columns .= $this->wrap($field) . ' =  CASE ' . $this->wrap($keyName);
            foreach ($arguments as $key => $item) {
                if (Arr::exists($item, $field)) {
                    $columns .= sprintf(' WHEN %s THEN %s', $this->parameter($key), $this->parameter($item[$field]));
                    array_push($bindings, $key, $item[$field]);
                }
            }
            $columns .= ' ELSE ' . $this->wrap($field) . ' END, ';
        }
        ...
        return ["update {$table} set {$columns} {$wheres}", $bindings];
    }
    ...
    public function prepareBindingsForUpdateBatch(array $bindings, array $values)
    {
        $cleanBindings = Arr::except($bindings, ['join', 'select']);
